...
class _CameraPageState extends State<CameraPage> {
  ...
  void initState() {
    ...
    int pic_len = context.read<ListBundle>().pictureList.length; // 홈페이지를 갔다 오면 슬라이더에 마지막 사진으로 이동하기 위해
    if (pic_len <= 0) {
      pic_len = 0;
    } else {
      pic_len = pic_len - 1;
  ...
  // }
  loadModel() async {
    await Tflite.loadModel(  // Tflite 객체에 모델 장착
  ...
  Future classifyImage(File image) async {
    ...
    int count_time = 0;
    while (count_time < 20) { // 0.1초씩 20번 돌려서 그사이에 output나오면 break
    ...
    setState(() {
      context.read<ListBundle>().add_list(label_value, conf_value, desc_value); // 리스트에 저장
  ...
  Widget build(BuildContext context) {
    if (!controller.value.isInitialized) { // if문은 카메라가 초기화 되지 않았을때 처리하는 if
...
//    debugShowCheckedModeBanner: false, // debug 라벨
    return Container( // 배경색을 top에서 bottom 까지 적용하기위해 Container안에 Scaffold를 넣음
        decoration: BoxDecoration( // 배경화면 gridient
            ...
            child: Center(
              child: Column(
                ...
                children: [
                  ...
                  Padding( // 갤러리에 저장, 체크박스, 사진버튼 을 한 Row에
                      ...
                      child: Row(
                          ...
                          children: [
                            Transform.scale(
                              ...
                              child: Checkbox(
                                  shape: RoundedRectangleBorder(
                                    borderRadius: BorderRadius.circular(2.0), // 체크박스 보더
                                  ),
                                  side: MaterialStateBorderSide.resolveWith(
                                        (states) => BorderSide(width: 2.0, color: Colors.white), // 체크박스 보더
                                  ),
                                  activeColor: Colors.white,
                                  checkColor: Colors.red,
                                  value: context
                                      .read<ListBundle>().gallery_save,
                                  onChanged: (value) {
                                    setState(() {
                                      context
                                          .read<ListBundle>().gallery_save = value!; // 체크 유무에 따라 mainpage의 gallery_save 값 변경
                                    });
                                  }),
                            ),
                            Text(
                            ...
                            ElevatedButton(  // 카메라 버튼
                              onPressed: () async {  // 누르면
                                if (camera_btn) {   // 2중 클릭 방지
                                  pictureFile = await controller.takePicture(); // 사진찍기
                                  setState(() {
                                    pic_exist = true;
                                    camera_btn = false;
                                  });
                                  ...
                                  if (pic_exist) {
                                    if (context.read<ListBundle>().pictureList.length > 20) { // 사진이 20장이 넘으면 앞에서 부터 지운다.
                                      context
                                          .read<ListBundle>()
                                          .del_first_pic();
                                      context
                                    ...
                                    context
                                        .read<ListBundle>()
                                        .add_pic_list(pictureFile!); // 사진을 리스트에 넣는다.
                                    context.read<ListBundle>().add_list(
                                        'should_be_changed',
                                        'should_be_changed',
                                        'should_be_changed'); // 모델의 output이 나오기 까지 시간이 걸려 label등의 값을 임의의 값으로 넣고 output이 나오면 update

                                    classifyImage(File(pictureFile!.path)); // 이미지 분류
                                  }
                                  var pList_len = context.read<ListBundle>().pictureList.length; //이미지 리스트의 길이
                                  ...
                                  if (context.read<ListBundle>().gallery_save) {  // 갤러리에 저장 하겠다고 체크되어있으면 저장
                                    GallerySaver.saveImage(pictureFile!.path).then((value) =>
                                            print('>>>> save value= $value'))
                                        .catchError((err) {
                                      print('error :( $err');
                                    });
                                  }
                                  if (context.read<ListBundle>().pictureList.length > 0) { //  사진리스트가 0이 아니면
                                    initState_page(pList_len);  // 슬라이더가 방금찍은것을 가리키도록
                                  }
                                }
                              },
                              style: ElevatedButton.styleFrom( // 카메라 모양 버튼
                                primary: is_loading == false
                                    ? Colors.indigoAccent
                                    : Colors.grey, // 찍고 모델에서 output이 아직 나오지 않았다면 gray
                  ...
                  context.read<ListBundle>().label_list.length > 0
                      ? is_loading == false // 아직 읽고있으면(output기달리면) 로딩중보여주고 끝났으면 값 보여주고
                          ? Container(
                              ...
                              Text(
                                ' desc : ${context.read<ListBundle>().desc_list[activePage]} \n \n \n \n \n \n \n  ${context.read<ListBundle>().desc_list[activePage]}  ',
                                style: TextStyle(
                                    color: Colors.white, fontSize: 20),
                              )
                            ]))
